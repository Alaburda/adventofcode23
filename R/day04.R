...
f04a <- function(x) {
  ...
  winning_numbers = str_extract_all(str_extract_all(string = x, pattern = ":\\s*([0-9\\s]+)\\|"), "\\d+")
  scratchcard_numbers = str_extract_all(str_extract_all(string = x, pattern = "\\|([0-9\\s]+)"), "\\d+")

  streak_length <- purrr::map2(scratchcard_numbers, winning_numbers, intersect) %>%
    map_chr(length) %>%
    as.numeric()

  out <- sum(2^(streak_length[streak_length > 0]-1))
...
f04b <- function(x) {
  ...
  copies <- rep(1,length(streak_length))

  for (i in 1:length(streak_length)) {
    if (streak_length[i] > 0) {
      copies[(i+1):(i+streak_length[i])] <- copies[(i+1):(i+streak_length[i])]+1*copies[i]
